...
- [ ] **advisory** The interface tests play a whole run through the screens to
      ...
      devices, in English and in Hebrew. They report rather than block, because
      they are timing-sensitive under CI load (see `ios/README.md`), so read
      their result: a green build does not assert that they passed.
